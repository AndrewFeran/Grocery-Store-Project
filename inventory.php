// Adding a check and alter table query to ensure the RestockRequest table has necessary columns
    try {
        // Check if AutoRestock column exists in RestockRequest table
        $column_check_sql = "SHOW COLUMNS FROM RestockRequest LIKE 'AutoRestock'";
        $column_check_stmt = $conn->prepare($column_check_sql);
        $column_check_stmt->execute();
        
        if ($column_check_stmt->rowCount() == 0) {
            // Add the AutoRestock column
            $alter_sql = "ALTER TABLE RestockRequest ADD COLUMN AutoRestock TINYINT(1) DEFAULT 0";
            $conn->exec($alter_sql);
        }
    } catch(PDOException $e) {
        // Silently ignore - might fail if user doesn't have ALTER privileges
        error_log("Failed to check/add AutoRestock column: " . $e->getMessage());
    }            // Auto-restock checkbox event listener
            document.getElementById('auto_restock').addEventListener('change', function() {
                // Set cookie to remember the setting (expires in 30 days)
                var expiryDate = new Date();
                expiryDate.setDate(expiryDate.getDate() + 30);
                document.cookie = "auto_restock=" + this.checked + "; expires=" + expiryDate.toUTCString() + "; path=/";
                
                // If turning on auto-restock, refresh the page to trigger the auto-restock check
                if (this.checked) {
                    location.reload();
                }
            });    // Check for products below threshold and create auto-restock requests if enabled
    if(isset($_COOKIE['auto_restock']) && $_COOKIE['auto_restock'] === 'true') {
        try {
            // Get all products with low stock (below 5)
            $low_stock_sql = "SELECT ID, Name, Quantity, Category, Buy_Price FROM Product WHERE Quantity > 0 AND Quantity < 5";
            $low_stock_stmt = $conn->prepare($low_stock_sql);
            $low_stock_stmt->execute();
            $low_stock_products = $low_stock_stmt->fetchAll(PDO::FETCH_ASSOC);
            
            // Check if any auto-restocks were already created today (to avoid duplicates)
            $today = date('Y-m-d');
            $check_sql = "SELECT DISTINCT Product_ID FROM RestockRequest 
                         WHERE RequestDate >= '$today 00:00:00' AND AutoRestock = 1";
            $check_stmt = $conn->prepare($check_sql);
            $check_stmt->execute();
            $already_restocked = $check_stmt->fetchAll(PDO::FETCH_COLUMN);
            
            $auto_restock_messages = [];
            
            foreach($low_stock_products as $product) {
                // Skip if already auto-restocked today
                if(in_array($product['ID'], $already_restocked)) {
                    continue;
                }
                ...
                $supplier_stmt->execute([$product['Category']]);
                ...
                // Calculate restock quantity (restock to 20 units)
                $restock_quantity = 20 - $product['Quantity'];
                ...
                try {
                    ...
                    $auto_restock_sql = "INSERT INTO RestockRequest 
                                       (Product_ID, Supplier_ID, Quantity, Status, RequestDate, AutoRestock) 
                                       VALUES (?, ?, ?, 'Pending', NOW(), 1)";
                    $auto_restock_stmt = $conn->prepare($auto_restock_sql);
                    $auto_restock_stmt->execute([$product['ID'], $supplier_id, $restock_quantity]);
                    
                    // Commit transaction
                    $conn->commit();
                    
                    // Add to success messages
                    $auto_restock_messages[] = "Auto-restock request created for " . htmlspecialchars($product['Name']) . 
                                              " (" . $restock_quantity . " units)";
                } catch(PDOException $e) {
                    // Rollback transaction in case of error
                    $conn->rollBack();
                    
                    // Log the error but don't display it to avoid cluttering the UI
                    error_log("Auto-restock error for product ID {$product['ID']}: " . $e->getMessage());
                }
            }
            
            // Show success message if any auto-restocks were created
            if (!empty($auto_restock_messages)) {
                $success_message = "Automatic restock requests created:<br>" . implode("<br>", $auto_restock_messages);
            }
            
        } catch(PDOException $e) {
            // Log the error but don't display it
            error_log("Auto-restock error: " . $e->getMessage());
        }
...
<body>
    ...
    <div class="store-balance-banner">
        ...
        <div style="margin-left: 20px;">
            <label for="auto_restock" style="cursor: pointer; font-weight: bold;">
                <input type="checkbox" id="auto_restock" name="auto_restock" 
                       <?php echo isset($_COOKIE['auto_restock']) && $_COOKIE['auto_restock'] === 'true' ? 'checked' : ''; ?>>
                Enable Automatic Restock When Below 5 Units
            </label>
        </div>
    </div>
